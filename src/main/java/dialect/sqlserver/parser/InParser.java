...
public class InParser implements SQLFilterParser {
    @Override
    public SQLQuery parse(final Filter filter, final Map<String, String> aliasMap) {
        ...
        return SQLQuery.builder()
                .query(fieldName + " IN (:" + filter.getName().toUpperCase() + ")")
                .arguments(Map.of(filter.getName().toUpperCase(), filter.getOperation().getValues()))
                .build();
    }
}
